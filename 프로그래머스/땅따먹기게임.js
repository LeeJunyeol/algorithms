...
function hopscotch(board, size) {
    ...
    // 2를 밟으면
    // 1 3 4 밟아보자.
    // i = 1, j = 0 일때, dp[1][0] = Math.max.apply(null, [dp[0][1], dp[0][2], dp[0][3])
    const dp = board.slice();
    for(let i = 1; i < size; i++){
        for(let j = 0; j < 4; j++){
            switch(j) {
                case 0 : {
                    dp[i][j] += Math.max.apply(null, [dp[i-1][1], dp[i-1][2], dp[i-1][3]]);
                    break; 
                }
                case 1 : {
                    dp[i][j] += Math.max.apply(null, [dp[i-1][0], dp[i-1][2], dp[i-1][3]]);
                    break; 
                }
                case 2 : {
                    dp[i][j] += Math.max.apply(null, [dp[i-1][0], dp[i-1][1], dp[i-1][3]]);
                    break; 
                }
                case 3 : {
                    dp[i][j] += Math.max.apply(null, [dp[i-1][0], dp[i-1][1], dp[i-1][2]]);
                    break;
                }
            }
        }
    ...
    result = Math.max.apply(null, dp[size - 1]);    
